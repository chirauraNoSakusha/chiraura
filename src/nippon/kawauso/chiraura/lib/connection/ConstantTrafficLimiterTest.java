 ...
 */
abstract class ConstantTrafficLimiterTest {
    private static final Logger LOG = Logger.getLogger(ConstantTrafficLimiterTest.class.getName());

    ConstantTrafficLimiterTest() {
        TestFunctions.testLogging(this.getClass().getName());
    }

    abstract TrafficLimiter getConstantTrafficLimiter(final long duration, final long sizeLimit, final int countLimit, long penalty);

    void something(final long duration, final long sizeLimit, final int countLimit, final int numOfThreads, final int numOfAddresses,
            final int numOfLoops, final long penalty) throws Exception {
        final TrafficLimiter controller = getConstantTrafficLimiter(duration, sizeLimit, countLimit, penalty);
