 ...
 */
final class PatchAndGetOrUpdateCacheReplyDriver {

    private static final Logger LOG = Logger.getLogger(PatchAndGetOrUpdateCacheReplyDriver.class.getName());

    // 参照。
    private final StorageWrapper storage;

    PatchAndGetOrUpdateCacheReplyDriver(final StorageWrapper storage) {
        if (storage == null) {
    ...
    <T extends Mountain> void execute(final PatchAndGetOrUpdateCacheReply<T> reply) throws InterruptedException {
        if (reply.isRejected() || reply.isGivenUp()) {
            ...
            if (cache != null && cache.getChunk().equals(reply.getChunk())) {
                LOG.log(Level.FINEST, "{0} により更新しました。", reply);
            } else {
                LOG.log(Level.FINEST, "{0} による更新はありませんでした。", reply);
            }
