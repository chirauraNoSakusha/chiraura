 ...
 */
final class AddCacheMessage implements Message {

    private final long type;
    private final Chunk chunk;

    private AddCacheMessage(final long type, final Chunk chunk) {
        if (chunk == null) {
    ...
    AddCacheMessage(final TypeRegistry<Chunk> chunkRegistry, final Chunk chunk) {
    ...
    static BytesConvertible.Parser<AddCacheMessage> getParser(final TypeRegistry<Chunk> chunkRegistry) {
        return new BytesConvertible.Parser<AddCacheMessage>() {
            @Override
            public int fromStream(final InputStream input, final int maxByteSize, final List<? super AddCacheMessage> output) throws MyRuleException,
                    IOException {
                ...
                output.add(new AddCacheMessage(type[0], chunk.get(0)));
                return size;
            }
        };
    }
    ...
    public boolean equals(final Object obj) {
        if (this == obj) {
            return true;
        } else if (!(obj instanceof AddCacheMessage)) {
            return false;
        }
        final AddCacheMessage other = (AddCacheMessage) obj;
